...
for d_i=1:length(days)  % Iterate through all of the specified days
    ...
    % Before iterating through all trials, remove those that don't match the stimLocsToAnalyze arg
    newTrialsToDo = [];
    if (isempty(stimLocsToAnalyze))
        newTrialsToDo = trialsToDo;
    else
        for t_i=1:length(trialsToDo)
            stimLocX = getStimLoc(actRecs, trialsToDo(t_i));
            if (~isempty(find(stimLocsToAnalyze == stimLocX, 1)))
