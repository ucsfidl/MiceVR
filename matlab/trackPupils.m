...
% With an update to the Arduino code on rig 1 (10/2020), one camera might have one extra frame than the other.
% This is OK and allowed, so account for it.
if (defVid == 2)
    totalFrames = v(defVid).NumberOfFrames;
else
    if (v(1).NumberOfFrames >= v(2).NumberOfFrames)
        totalFrames = v(2).NumberOfFrames;
    else
        totalFrames = v(1).NumberOfFrames;
    end
end
...
while relFrame + frameStart <= frameStop + 1
    ...
    for i=startVid:stopVid  % 1 is L, 2 is R
        ...
        %%% Adapted from pupil-finding code above, but simpler as can assume CR is saturating white %%%
        if (crPresent(i))
            subIm = im2bw(imLR(:,:,:,i), crMinVal / 255);
            cc = bwconncomp(subIm);
            ...
            if (~isempty (cc.PixelIdxList))
                ...
                if (~isempty(s))
                    ...
                    if (relFrame > crPosHistLen)
                        ...
                        avgPrevPos = mean(cNoNaN, 1);
                        if (~isempty(cNoNaN))
                            lastKnownPos = cNoNaN(size(cNoNaN,1), :);
                            foundPrevCR = 1;
                        else  % if CR never observed even though we are past the 1st frame, default to the center pos
                            lastKnownPos = [v(defVid).Width/2 v(defVid).Height/2]; 
                        end
                    else
                        % CR will tend to be in the center of the screen, so initialize to there.
                        % This sometimes causes problems for pupils so we don't do, but use for CRs
                        lastKnownPos = [v(defVid).Width/2 v(defVid).Height/2];
                        avgPrevPos = lastKnownPos;
                    end
                    avgHistPos = repmat(avgPrevPos, size(curPos, 1), 1);
                    distFromHistPos = sqrt(power(curPos(:,1) - avgHistPos(:,1),2) + power(curPos(:,2) - avgHistPos(:,2),2))';
                    distFromLastKnownPos = sqrt(power(curPos(:,1) - lastKnownPos(:,1),2) + power(curPos(:,2) - lastKnownPos(:,2),2))';

                    % If we have accumulated enough history, censor CRs that are too far from the historical position
                    ...
                    if (relFrame > 1 && foundPrevCR)
                        ...
                        if (length(idx) > 1)
                            ...
                            distFromHistPos = distFromHistPos(idx);
                            cA = cA(idx);
                            [~,idx2] = max( crSolWt * solidity/max(solidity) + ...
                                           crDistWt * min(distFromHistPos) ./ (distFromHistPos + distFudge) + ...
                                           crSizeWt * cA/max(cA));
                            idx = idx(idx2);
                        end
                    else
