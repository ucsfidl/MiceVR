...
public class ArduinoComm : MonoBehaviour
{
    ...
    public void init()
    {
		...
		this.usbWriter.ReadTimeout = 10;
		this.usbWriter.WriteTimeout = 10;
	...
	public void sendReward(int lickportID, int dur) {
		try {
			byte[] data = BitConverter.GetBytes(lickportID);

			if (!usbWriter.IsOpen)
				usbWriter.Open();

			usbWriter.Write(lickportID + " " + dur);
			Debug.Log("sent to arduino: " + lickportID + " " + dur);
		}
		catch (Exception err)
		{
			Debug.Log(err.ToString());
